...
static mcp3008_t controller0;
static mcp23s08_t controller1;
...
void synth_controller_init() {
  // use CS1
  controller1 =
      synth_mcp23s08_init(spi0, BAUD_RATE, SPI_CS1, SPI_CLOCK, SPI_TX, SPI_RX);
...
static void read(audio_context_t* context) {
  ...
  for (size_t i = 0; i < CONTROLS_COUNT; i++) {
    uint16_t value =
        snap(synth_mcp3008_read(&controller0, controls[i].channel));
