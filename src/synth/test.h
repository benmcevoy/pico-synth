...
static void set_frequency(AudioContext_t* context, fix16 f,
    ...
    for (int v = 0; v < VOICES_LENGTH; v++) {
        ...
        synth_audiocontext_set_wavetable_stride(voice, context->sampleRate);
    }
}
...
static void test_pattern(AudioContext_t* context, Waveform_t waveform) {
...
static void test_midi_pattern(AudioContext_t* context, Waveform_t waveform) {
